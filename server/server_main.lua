...
local function closeBag(source, id)
     ...
     local stash_items = Harmony.Stash('Bag_' .. id, id).Items('inventory')
     ...
     Harmony.Stash('Bag_' .. id, id).Save(stash_items)
...
local function openBag(src, id, item_name)
     ...
     local stash = Harmony.Stash('Bag_' .. id, id)
...
RegisterNetEvent('keep-harmony:stash:opening->cancel', function(prefix, id)
     if prefix ~= 'Bag_' .. id then return end
